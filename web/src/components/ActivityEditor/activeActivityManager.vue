  ...
  <div>
    ...
    <el-table :data="activities">
      ...
      <el-table-column label="操作" width="200">
        <template slot-scope="scope">
          <el-button v-if="!needEdit(scope.row.status)" size="mini" type="primary" @click="openControllerLink(scope.row.id)">
            <i class="el-icon-set-up"/>打开控制面板
          </el-button>
          <el-button v-else size="mini" @click="openEditLink(scope.row.id)">
...
export default class ActiveActivityManager extends Vue {
  ...
  private needEdit(status: ActivityStatus) {
    return [ActivityStatus.Unpassed, ActivityStatus.Pending].includes(status);
  }

  private openControllerLink(activityId: number) {
    const url = this.$router.resolve({
      path: `/controller/${activityId}`,
    });
    window.open(url.href, '_blank');
  }

  private openEditLink(activityId: number) {
    const url = this.$router.resolve({
